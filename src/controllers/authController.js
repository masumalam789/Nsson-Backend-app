...
exports.register = async (req, res) => {
  try {
    const { email, password, confirmPassword, firstName, lastName, phone, shopDetails } = req.body;
    ...
    if (!emailResult.success) {
      console.error('[Auth] registration received email failed:', emailResult.error);
    }
...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (user.role === 'admin') {
      const [totalUsers, totalProducts, totalOrders, totalCategories, pendingOrders, lowStockProducts, pendingUsers] =
        await Promise.all([
          ...
          User.countDocuments({ role: 'customer', status: 'pending' }), // ← pending approvals count
        ]);
      ...
      return res.status(200).json({
        ...
        stats: {
          ...
          pendingOrders,
          lowStockProducts,
          pendingUsers, // ← handy for admin dashboard badge
        },
        role: 'admin',
      });
    }
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (!result.success) {
      ...
      await user.save();
      console.error('[Auth] forgotPassword — email delivery failed:', result.error);
...
exports.verifyResetToken = async (req, res) => {
  try {
    ...
    return res.status(200).json({ valid: true, message: 'Token is valid', email: user.email });
  } catch (error) {
...
exports.registerAdmin = async (req, res) => {
  try {
    const { email, password, confirmPassword, firstName, lastName, phone, adminSecret } = req.body;
...
exports.loginAdmin = async (req, res) => {
    console.log('🔥 NEW loginAdmin called, body:', req.body); // ✅ add this
 ...
 console.log('🔍 user found:', user ? user.email || user.phone : 'NOT FOUND');  // ✅ add
    console.log('🔍 user role:', user?.role);   // ✅ add
    console.log('🔍 user status:', user?.status); // ✅ add
    const dummyHash = '$2a$12$invalidhashforcomparison000000000000000000000000000000';
    const isPasswordValid = user
      ? await bcrypt.compare(password, user.password)
      : await bcrypt.compare(password, dummyHash);
