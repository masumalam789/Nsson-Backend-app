...
exports.createCoupon = async (req, res) => {
  try {
    ...
    if (discountType === 'flat' && discountValue < 0) {
      return res.status(400).json({ success: false, error: 'Flat discount cannot be negative' });
    }

    if (minOrderAmount !== undefined && minOrderAmount < 0) {
      return res.status(400).json({ success: false, error: 'Minimum order amount cannot be negative' });
    }
    ...
    if (coupon.couponType === 'private' && userIds && Array.isArray(userIds) && userIds.length > 0) {
      ...
      if (validUserIds.length > 0) {
        ...
        await notificationService.notifyUsers(validUserIds, payload, { createdBy: req.user?._id });
      }
    }
...
exports.getCouponById = async (req, res) => {
  try {
    ...
    const assignments = await UserCoupon.find({ couponId: coupon._id }).select('userId');
  ...
  try {
    ...
    if (coupon.couponType === 'private' && userIds && Array.isArray(userIds)) {
      ...
      if (toRemove.length > 0) {
        await UserCoupon.deleteMany({ couponId: coupon._id, userId: { $in: toRemove } });
      }
    ...
    return res.status(500).json({ success: false, error: error.message || 'Failed to update coupon' });
  }
};
...
exports.getAvailableCoupons = async (req, res) => {
  try {
    ...
    const coupons = await Coupon.find({
      ...
      .select('code title description discountType discountValue minOrderAmount maxDiscountAmount startDate endDate usageLimit usedCount couponType isActive')
      .sort({ createdAt: -1 });
...
exports.applyCoupon = async (req, res) => {
  try {
    ...
    const { coupon, discountAmount, finalAmount } = await couponService.validateAndCalculate(
      code,
      Number(orderAmount),
      req.user?._id
    );
    ...
    return res.status(400).json({ success: false, error: error.message || 'Failed to apply coupon' });
  }
};
...
exports.assignCoupon = async (req, res) => {
  try {
    ...
    if (!userIds || !Array.isArray(userIds) || userIds.length === 0) {
      return res.status(400).json({ success: false, error: 'User IDs are required and must be an array' });
    }
    ...
    if (coupon.couponType !== 'private') {
      return res.status(400).json({ success: false, error: 'Only private coupons can be assigned to users' });
    }
    ...
    if (validUserIds.length === 0) {
      return res.status(400).json({ success: false, error: 'No valid active customers found to assign' });
    }
    ...
    return res.status(500).json({ success: false, error: error.message || 'Failed to assign coupon' });
  }
};
...
exports.getMyAssignedCoupons = async (req, res) => {
  try {
    ...
    const now = new Date();
    // Coupons expired more than 15 days ago are dropped from history entirely
    const cutoff = new Date(now.getTime() - 15 * 24 * 60 * 60 * 1000);

    // Fetch all assignments (used or not) whose coupon endDate is either null or >= cutoff
    const assignments = await UserCoupon.find({ userId })
      .populate({
        path: 'couponId',
        select: 'code title description discountType discountValue minOrderAmount maxDiscountAmount startDate endDate usageLimit usedCount couponType isActive',
        match: {
          // Drop coupons that expired more than 15 days ago
          $or: [{ endDate: null }, { endDate: { $gte: cutoff } }],
        },
      })
      .sort({ createdAt: -1 });

    // Build response with a computed `status` field the app can use directly
    const coupons = assignments
      .filter(a => a.couponId) // drop null (didn't match the populate match)
      .map(a => {
        const c = a.couponId.toObject();
        const isUsed = a.isUsed;
        const expired = c.endDate && now > new Date(c.endDate);
        const notStarted = c.startDate && now < new Date(c.startDate);
        const exhausted = c.usageLimit !== null && c.usageLimit !== undefined && c.usedCount >= c.usageLimit;
        ...
        return {
          ...
          isUsed,
          status,          // 'active' | 'used' | 'expired' | 'upcoming' | 'exhausted' | 'inactive'
    ...
    return res.status(500).json({ success: false, error: 'Failed to fetch your coupons' });
  }
};