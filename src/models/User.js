...
// ─── Pre-validate hook ────────────────────────────────────────────────────────
// NOTE: Only enforce strict field requirements on NEW document creation.
// Updates (findByIdAndUpdate with runValidators) should not re-validate
// fields that aren't part of the partial update payload.
...
userSchema.pre("validate", async function () {
  ...
  if (this.role === "customer") {
    ...
    // Only enforce required fields on brand-new documents, not on partial updates
    if (this.isNew) {
      if (!this.address) {
        this.invalidate("address", "Address is required for customers.");
      }

      // Shop details are required for new customers
      if (!this.shopDetails?.shopName) {
        this.invalidate(
          "shopDetails.shopName",
          "Shop name is required for customers",
        );
      }
      if (!this.shopDetails?.businessAddress) {
        this.invalidate(
          "shopDetails.businessAddress",
          "Business address is required for customers",
        );
      }
    }
  }
